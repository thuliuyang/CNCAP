		...
		for j := 0; j < T.Shape[1]; j++ {
			go func(j int) { // concurrent processing
				for i := 0; i < T.Shape[2]; i++ {
					...
					//fmt.Println(k,j,i,T.Get(k, j, i), P.Get(k, j, i))
					}
					//fmt.Println(k,LayerHeights.Shape[0],LayerHeights.Get(0,j,i),LayerHeights.Get(14,j,i),pblh.Get(j,i),pblTop,To)
					...
					//t97,t98,t99:=DryDepGas(z, zo, u, L, To,  ρ,
                                       //                 G, Θsurface,
                                      //                  wesely1989.So2Data, iSeasonG,
                                        //                weselyLU, rain, dew, true, false)
					//fmt.Println(t97,t98,t99,T.Shape[1],T.Shape[2],j,i)
				//	oo1:=T.Get(0, j, i)
				 //       oo2:=P.Get(0, j, i)
					//fmt.Println(oo1,oo2,k,pblTop,To,z, zo, u, L, To, ρ, G, Θsurface,wesely1989.So2Data, iSeasonG,weselyLU, rain, dew)
					SO2DryDep.AddVal(
					...
					for k := 0; k < T.Shape[0]; k++ {
						...
						kso2 := SO2rate * hoConc
					//	fmt.Println("gaseous so2 oxidation")
					//	fmt.Println(SO2rate)
					//	fmt.Println(hoConc)
					//	fmt.Println(kso2) 
						...
						if qCloudVal > 0. {
							...
							kso2 += seinfeld.SulfurH2O2aqueousOxidationRate(
								...
								qCloudVal)
                                              ...
                                              //  fmt.Println("add so2 oxidized by h2o2")					     
					//	fmt.Println(qCloudVal)
					//	fmt.Println(kso2)
...
func DryDepGas(z, zo, ustar, L, T, rhoA, G, Θ float64,
    ...
    rain, dew, isSO2, isO3 bool) (float64,float64,float64) {
    Ra := ra(z, zo, ustar, L)
    Mu := mu(T)
