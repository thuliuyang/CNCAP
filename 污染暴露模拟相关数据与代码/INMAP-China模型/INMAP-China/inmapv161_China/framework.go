...
func SetTimestepCFL() DomainManipulator {
	...
	return func(d *InMAP) error {
		...
		for _, c := range *d.cells {
			// Advection time step
			cUadv := (math.Abs(c.UAvg) + c.UDeviation*2) / c.Dx/1000
			cVadv := (math.Abs(c.VAvg) + c.VDeviation*2) / c.Dy/1000
			cWadv := math.Abs(c.WAvg) / c.Dz
			// horizontal diffusion time step
			cXdiff := 2. * c.Kxxyy / (c.Dx * c.Dx/1000000)
			cYdiff := 2. * c.Kxxyy / (c.Dy * c.Dy/1000000)
